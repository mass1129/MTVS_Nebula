...
namespace MD_Plugin
{
    ...
    public class MDM_FFD : MonoBehaviour
    {
        ...
        public float ppFFDOffsetMultiplier = 1.0f;
        ...
        public void FFD_RegisterWeights()
        {
            ...
            Vector3 maxBounds = transform.position + Vector3.Scale(transform.localScale, meshFilter.sharedMesh.bounds.max) + (Vector3.one * (ppFFDOffset * ppFFDOffsetMultiplier));
            Vector3 minBounds = transform.position + Vector3.Scale(transform.localScale, meshFilter.sharedMesh.bounds.min) - (Vector3.one * (ppFFDOffset * ppFFDOffsetMultiplier));
...
namespace MD_PluginEditor
{
    ...
    public class MDM_FFDEditor : MD_EditorUtilities
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            pve();
            ph();
            if (pb(new GUIContent("Refresh FFD Grid","Refresh current FFD grid - the mesh will switch to its initial state")))
                targ.FFD_RefreshFFDGrid();
            if (pb(new GUIContent("Clear FFD Grid", "Clear current FFD grid - the mesh status will remain and the grid will be removed")))
                targ.FFD_ClearFFDGrid();
            phe();
            ps(5);
            ppDrawProperty("ppFFDNodeSize", "Node Size");
            ppDrawProperty("ppFFDOffset", "FFD Grid Offset");
            ppDrawProperty("ppFFDOffsetMultiplier", "Offset Multiplier", identOffset:true);
            pve();
            ps();
            ...
            ps(5);
            pv();
            if (pb(new GUIContent("Register Weights Manually","Register current mesh status (override initial vertices) and continue in the mesh editing") ))
