...
namespace MD_Plugin
{
    ...
    public class MDM_Morpher : MonoBehaviour
    {
        ...
        [System.Serializable]
        public class RegisteredMorphShapes
        {
            public List<Vector3> vertices = new List<Vector3>();
            public List<int> indexes = new List<int>();
        }
        public List<RegisteredMorphShapes> ppRegisteredMorphs = new List<RegisteredMorphShapes>();

        internal bool modeSwitch = false;
        ...
        private void Start()
        {
            if (ppMultithreadingSupported && Application.isPlaying)
            {
                ...
                otherThread = new Thread(ThreadWorker_UpdateMorpher);
                otherThread.Start();
            }
        }
        ...
        public void Morpher_RefreshTargetMeshes()
        {
            if (ppRegisteredMorphs == null)
                ppRegisteredMorphs = new List<RegisteredMorphShapes>();
            else
            ...
            foreach (Mesh m in ppTargetMorphMeshes)
            {
                ...
                RegisteredMorphShapes regMorph = new RegisteredMorphShapes();
        ...
        {
            if (otherThread != null && otherThread.IsAlive)
                otherThread.Abort();
        }
