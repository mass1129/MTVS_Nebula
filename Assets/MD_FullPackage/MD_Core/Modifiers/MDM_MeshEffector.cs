...
namespace MD_Plugin
{
    ...
    /// Deform mesh by the specific weight values & effector attributes. Ready for complex meshes
    /// </summary>
    [ExecuteInEditMode]
    ...
    public class MDM_MeshEffector : MonoBehaviour
    {
        ...
        private bool initialVertsInitialized = false;
        [SerializeField] private Vector3[] initialVerts;
        ...
        private void Awake()
        {
            ...
            if (!initialVertsInitialized)
            {
                initialVerts = meshFilter.sharedMesh.vertices;
                initialVertsInitialized = true;
            }
        }
        ...
        /// </summary>
        public void Effector_RestoreMesh()
        {
            meshFilter.sharedMesh.vertices = initialVerts;
            ppGeneratedPoints = meshFilter.sharedMesh.vertices;
            ppGeneratedPointsOrigins.Clear();
            for (int i = 0; i < ppGeneratedPoints.Length; i++)
                ppGeneratedPointsOrigins.Add(transform.TransformPoint(ppGeneratedPoints[i]));
...
namespace MD_PluginEditor
{
    ...
    public class MDM_MeshEffectorEditor : MD_EditorUtilities
    {
        ...
        public override void OnInspectorGUI()
        {
            ps();
            pv();
            ppDrawProperty("ppUpdateEveryFrame", "Update Every Frame", "If enabled, the script will be updated every frame");
            pve();
            ...
            if(pb("Restore Mesh"))
            {
                if(pdd("Are you sure?","Are you sure to restore the current mesh to its initial state? There is no way back...","Yes","No"))
                    targ.Effector_RestoreMesh();
            }
            pve();
