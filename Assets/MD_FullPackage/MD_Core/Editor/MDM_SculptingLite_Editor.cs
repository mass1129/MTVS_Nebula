...
namespace MD_PluginEditor
{
    ...
    public class MDM_SculptingLite_Editor : MD_EditorUtilities
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if(ss.SS_AutoRecalculateNormals)
            {
                pplus();
                ppDrawProperty("SS_AlternateNormalsRecalculation", "Alternate Normals", "Use alternate normals recalculation - results are better, but takes more performance (Great for spatial meshes)");
                if(ss.SS_AlternateNormalsRecalculation)
                    ppDrawProperty("SS_AlternateNormalsRecalculationAngle", "Angle", "Average angle of the alternate normals recalculation");
                pminus();
            }
            ...
            if (ss.SS_RecordHistory)
            {
                ...
                phb("The history will be automatically recorded on any control up. Call 'Undo' method to make a step backward or forward");
            }
            pve();
            pve();
            ...
            if (!ss.SS_AtRuntime)
            {
                ...
                if(!EditorWindow.GetWindow<SceneView>().drawGizmos)
                    phb("To make the sculpting in editor work - Gizmos must be enabled!", MessageType.Warning);
            ...
            ps(15);
            pl("Interested in Pro version?", true);
            if (pb("Sculpting Pro"))
