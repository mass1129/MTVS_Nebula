    ...
    public class MD_VertexTool : MD_EditorWindowUtilities
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button(new GUIContent("Attach", _Attach)))
            {
                if (!Active)
                    return;
                MD_VertexToolModifier.V_TOOL_Element_Attach(Selection.gameObjects[0], !EditorUtility.DisplayDialog("Question","Would you like to attach current selection or children of the first selected object?","Selection","Children"));
            }
            GUILayout.Space(5);
            ...
            if (GUILayout.Button(new GUIContent("Clone", _Clone)))
            {
                ...
                if (Selection.gameObjects.Length == 0)
                    return;
                MD_VertexToolModifier.V_TOOL_Element_Clone(Selection.gameObjects[0], MultiplyCounter, MultiplyAngle, MultiplyMoveOffset);
            }
...
namespace MD_Plugin
{
    ...
    {
        /// <summary>
        /// Attach 2 or more meshes [Meshes will be combined and will share the same material]​. If attachCildren is true, the RootObjects children will be combined with it. Otherwise the objects selection will be combined (editor only).
        /// </summary>
        public static void V_TOOL_Element_Attach(GameObject RootObject, bool attachChildren = true)
        {
            ...
            if (attachChildren)
            {
                foreach (MeshFilter gm in RootObject.GetComponentsInChildren<MeshFilter>())
                {
                    ...
                    {
                        gm.GetComponent<MD_MeshProEditor>().ppAnimationMode = false;
                        gm.GetComponent<MD_MeshProEditor>().MPE_ClearVerticeEditor();
                    }
                    if (!gm) continue;

                    if (AttachRoot == gm.gameObject) continue;
        ...
        {
#if UNITY_EDITOR
            Undo.RegisterCompleteObjectUndo(new Object[2] { PointA, PointA.gameObject }, "Weld Points");
            Undo.SetTransformParent(PointA, PointB, "Weld Points [Parent]");
#endif
        ...
        public static void V_TOOL_Vertex_Relax(MD_MeshProEditor Sender)
        {
            ...
            {
#if UNITY_EDITOR
                Undo.RegisterCompleteObjectUndo(Sender.workingTargetPoints.ToArray(), "Relax Points");
#endif
