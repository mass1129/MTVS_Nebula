...
namespace MD_PluginEditor
{
    ...
    public class MD_EditorUtilities : Editor
    {
        ...
        protected void ppAddMeshCollider(GameObject sender)
        {
            if (sender.GetComponent<MeshCollider>()) return;
            ColorUtility.TryParseHtmlString("#a6e0b5", out Color c);
            GUI.color = c;
            if (pb("Add Mesh Collider")) sender.AddComponent<MeshCollider>();
        }
        ...
        protected bool pdd(string title, string msg, string ok = "Ok", string no = "")
        {
        ...
        protected void pplus()
        {
            EditorGUI.indentLevel++;
        }
        protected void pminus()
        {
            EditorGUI.indentLevel--;
        }
    }
