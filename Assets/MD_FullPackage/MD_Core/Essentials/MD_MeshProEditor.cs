...
namespace MD_Plugin
{
    ...
    public class MD_MeshProEditor : MonoBehaviour 
    {
		...
		public float ppVerticesSizeMultiplier = 1.0f;
        #endregion
        ...
        private void Awake () 
        {
            ...
            if (!_AlreadyAwake)
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    {
                        if (UnityEditor.EditorUtility.DisplayDialog("Create a New Reference?", "Would you like to create a new reference? If you agree [recommended], you will create a new mesh reference. If you disagree, existing mesh references will share the same data as the new mesh reference...", "Yes", "No"))
        ...
        public static class MeshProEditor_Utilities
        {
            ...
            public static bool Util_CheckFellowModifiers(GameObject sender, MonoBehaviour senderBehaviour)
            {
                foreach (MonoBehaviour beh in sender.GetComponents<MonoBehaviour>())
                {
                    ...
                    {
                        #if UNITY_EDITOR
                            if (!Application.isPlaying) UnityEditor.EditorUtility.DisplayDialog("Warning", "The modifier cannot be applied to this object, because the object already contains other modifiers. Please remove the existing modifiers to access the selected modifier.", "OK");
                        #else
            ...
            public static Mesh Util_CreateNewMeshReference(Mesh entryMesh)
            {
                return Instantiate(entryMesh);
            }

            /// <summary>
            /// Create brand new mesh reference, does some additional work (if shared mesh exists)
            /// </summary>
            public static void Util_CreateNewMeshReference(MeshFilter entryMeshFilter)
            {
                ...
                entryMeshFilter.sharedMesh = Util_CreateNewMeshReference(entryMeshFilter.sharedMesh);
            }
            ...
            public static void Util_PrepareMeshDeformationModifier(MonoBehaviour sender, MeshFilter senderFilter, bool checkVertCount = true)
            {
                ...
                if(MD_GlobalPreferences.CreateNewReference)
                    Util_CreateNewMeshReference(senderFilter);
        ...
        private void Internal_MPE_ResetReference()
        {
            ...
            Mesh newMesh = MeshProEditor_Utilities.Util_CreateNewMeshReference(meshFilter.sharedMesh);
       ...
       /// </summary>
        private void Internal_MPE_ReceiveMeshInfo(bool passAlreadyAwake = false)
        {
            ...
            if (!_AlreadyAwake || passAlreadyAwake)
            {
				...
				originalMesh = Instantiate(myMesh);
                originalMesh.name = myMesh.name;
            }
        }
        ...
        public void Internal_MPE_SaveMeshToAssetsDatabase()
        {
            ...
            string path = UnityEditor.EditorUtility.SaveFilePanelInProject("Please enter the path to save your Mesh to the Assets as Prefab", ppINFO_MeshName, "asset", "Please enter path");
            ...
            {
                Debug.LogError(e.Message + ", [Error while saving the asset...] Unique Path: " + UniquePath);
            }
        }
        ...
        public void MPE_CreateVerticeEditor(bool PassTheVerticeLimit = false)
        {
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                ...
                if(ppVerticesSizeMultiplier != 1)
                    gm.transform.localScale = Vector3.one * ppVerticesSizeMultiplier;
