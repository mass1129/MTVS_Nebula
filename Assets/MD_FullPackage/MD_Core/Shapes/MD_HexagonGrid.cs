...
namespace MD_Plugin
{
    ...
    public class MD_HexagonGrid : MonoBehaviour
    {
        ...
        public float ppMaximumHeightRange = 1;
        public float ppMiminumHeightRange = 0.1f;

        public bool ppPlanerHexagon = true;
        public bool ppInvert = false;

        public bool ppUpdateEveryFrame = true;
        public bool ppAlternateNormals = true;
        ...
        /// <summary>
        /// Essential method for the Hexagon Grid modification in general
        /// </summary>
        ...
        /// <summary>
        /// Modify a simple planar Hexagon Grid
        /// </summary>
        public void HexagonGrid_ModifyPlanar()
        {
            ...
            if (ppAlternateNormals)
                MD_MeshMathUtilities.AlternativeNormalCalculation.RecalculateNormals(m, ppAlternateNormalsAngle);
            else
        ...
        /// </summary>
        public void HexagonGrid_ModifySpatial(float min = 0, float max = 0, float processValue = 0)
        {
            Mesh m = new Mesh();
            m.name = "HexaGrid";
        ...
        /// <summary>
        /// Randomize hexagon grid height by specific min and max values
        /// </summary>
        public void HexagonGrid_RandomizeHeight(float min, float max)
        {
            HexagonGrid_ModifySpatial(min, max);
        }

        /// <summary>
        /// Randomize hexagon grid height by uniform value
        /// </summary>
        public void HexagonGrid_RandomizeHeight(float uniformValue)
        {
            HexagonGrid_RandomizeHeight(0.1f, uniformValue);
        }

        private float[] randomizedHeights;
        /// <summary>
        /// React the hexagon grid with audio - requires spacial hexagon grid
        /// </summary>
        public void HexagonGrid_SoundReact(MDM_SoundReact mdmsr)
        {
            if(randomizedHeights == null)
                randomizedHeights = new float[ppCount * ppCount];
            HexagonGrid_ModifySpatial(ppMiminumHeightRange, ppMaximumHeightRange, mdmsr.PpOutputData);
        }
    }
}
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!hg.ppUpdateEveryFrame && !hg.ppPlanerHexagon)
            {
                ...
                if (pb("Randomize Height"))
                    hg.HexagonGrid_RandomizeHeight(hg.ppMiminumHeightRange, hg.ppMaximumHeightRange);
                ppDrawProperty("ppMaximumHeightRange", "Max Random Height Value");
                ppDrawProperty("ppMiminumHeightRange", "Min Random Height Value");
